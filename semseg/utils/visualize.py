...
def draw_text(image: torch.Tensor, seg_map: torch.Tensor, labels: list, fontsize: int = 15):
    ...
    #font = ImageFont.truetype(r"C:\final_project_eran_nadav\semantic-segmentation-main\assests\Helvetica.ttf", fontsize)
    ...
    for idx, cls in zip(indices, classes):
        ...
        bbox = draw.textbbox(center, cls)
        bbox = (bbox[0]-3, bbox[1]-3, bbox[2]+3, bbox[3]+3)
        draw.rectangle(bbox, fill=(255, 255, 255), width=1)
        draw.text(center, cls, fill=(0, 0, 0))
    return pil_image