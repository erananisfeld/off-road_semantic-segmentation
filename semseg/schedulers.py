...
class eran_sched(WarmupLR):
    def __init__(self, optimizer, power, max_iter, eta_ratio=0.1, warmup_iter=500, warmup_ratio=5e-4, warmup='exp', last_epoch=-1) -> None:
        self.eta_ratio = eta_ratio
        self.max_iter = max_iter
        self.power = power
        super().__init__(optimizer, warmup_iter, warmup_ratio, warmup, last_epoch)
    ...
    def get_main_ratio(self):
        ...
        result = (1 - alpha) ** self.power * (1 + math.cos(math.pi * self.last_epoch / (self.eta_ratio * real_max_iter))) / 2
        return result

__all__ = ['polylr', 'warmuppolylr', 'warmupcosinelr', 'warmupsteplr', 'eran_sched']


def get_scheduler(scheduler_name: str, optimizer, max_iter: int, power: int, warmup_iter: int, warmup_ratio: float, eta_ratio=0.1):
    ...
    elif scheduler_name == 'eran_sched':
        return eran_sched(optimizer, power, max_iter, eta_ratio=eta_ratio, warmup_iter=warmup_iter, warmup_ratio=warmup_ratio)
